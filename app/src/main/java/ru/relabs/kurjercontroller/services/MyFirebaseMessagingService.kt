...
class MyFirebaseMessagingService : FirebaseMessagingService() {
    ...
    override fun onNewToken(pushToken: String) {
        ...
        bgScope.launch(Dispatchers.Default) {
            (application as? ControllApplication)?.let {
    ...
    suspend fun processMessageData(data: Map<String, String>) {
        if (data.containsKey("request_gps")) {
            val user = (application as? ControllApplication)?.user?.getUserCredentials() ?: return
            bgScope.launch(Dispatchers.Default) {
                val coordinates = ControllApplication.instance.currentLocation
        ...
        if (data.containsKey("closed_task_id") && data.containsKey("deliveryman_task")) {
            withContext(Dispatchers.IO) {
                ...
                val items = ControllApplication.instance.tasksRepository.getTaskItems(taskItemId)
                if (items.isNotEmpty()) {
                    items.forEach {
                        if (ControllApplication.instance.database.taskDao().getById(it.taskId)?.state != TaskModel.CREATED) {
